...
def to_tensor_obs_left_only(obs) -> TensorBimanualObs:
    ...
    return TensorBimanualObs(visual=visual, qpos=TensorBimanualState(torch.tensor(obs.qpos.array, dtype=torch.float32, device=DEVICE)[None]), qvel=qvel)
...
for step in tqdm(range(MAX_STEPS)):
    ...
    with torch.no_grad():
        ...
        if RELATIVE_ACTIONS:
            mask = np.ones_like(action, dtype=np.bool)
            mask[6] = 0
            mask[13] = 0
            action[mask] += obs.qpos.to_approximate_action().array[mask]
            # gripper thresholding
            action[6] = 0 if action[6] < 0.25 else 0.36
            # action[6] *= .1
